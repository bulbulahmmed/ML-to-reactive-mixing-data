...
#***********************************************;
''' LassoCV works better than gridsearchCV for Lasso model '''
import time
import numpy as np
import pandas as pd
...
param_grid = {"alpha": [1e0, 1e2, 1e3],
              "max_iter": [50, 300, 1000],
              "tol": [0.0001, 0.00001],
              "selection": ['cyclic', 'random']}
alphas      = np.logspace(-4, -0.5, 300)
model       = LassoCV(alphas=alphas, cv=10, random_state=42, max_iter=10000)
model_fit   = model.fit(X_train, y_train)
mse         = model.mse_path_.mean(axis=1)  # MSE error is equal to alpha number
min_mse     = mse.min()
#
...
print('Minimum alpha is: ', min_mse)
print('Alpha for min alpha is: ', alphas[])
#==================================================================================;
...
filename    = 'lasso_grid_search.txt'
